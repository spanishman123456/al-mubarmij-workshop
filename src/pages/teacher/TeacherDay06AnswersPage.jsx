...
import { fetchTeacherDay06AnswersApi } from "../../lib/platformApi";
import { day06TeacherAnswers } from "../../content/teacher/day06TeacherAnswers";
...
export default function TeacherDay06AnswersPage() {
  const [data, setData] = useState(day06TeacherAnswers);

  useEffect(() => {
    let cancelled = false;
    fetchTeacherDay06AnswersApi()
      .then((res) => {
  ...
  return (
    <PageShell title={titleAr} subtitle="للمعلم فقط — pdfPage 295–338">
      ...
      {teacherGuidance ? (
        <EduCard title={teacherGuidance.titleAr} accent="violet">
          ...
          <p className="mt-2 text-xs text-slate-600">{teacherGuidance.materialsAr}</p>
          <p className="mt-1 text-xs text-slate-600">التقييم: {teacherGuidance.assessmentAr}</p>
        </EduCard>
      ) : null}
      ...
      <div className="mt-6 space-y-6">
        {sections.map((sec) => (
          <EduCard key={sec.id} title={sec.titleAr} accent="amber">
            <p className="mt-1 text-xs text-slate-500">pdfPageIndex {sec.pdfPageIndex}</p>
            {sec.lessonRoute ? (
