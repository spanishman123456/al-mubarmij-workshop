...
<style>
:root{--c-bg:#0f172a;--c-panel:#172033;--c-text:#f8fafc;--c-muted:#94a3b8;--c-primary:#7c3aed;--c-primary2:#4f46e5;--c-ok:#10b981;--c-warn:#f59e0b;--c-danger:#ef4444;--space:1rem;--radius:.85rem;--shadow:0 14px 35px rgba(0,0,0,.25);color-scheme:dark}
*{box-sizing:border-box}body{margin:0;min-height:100vh;background:linear-gradient(150deg,#0f172a,#1e1b4b);color:var(--c-text);font-family:Tajawal,"Segoe UI",sans-serif}.root{min-height:100vh;padding:1rem}.empty{display:grid;place-items:center;min-height:220px;color:var(--c-muted);border:1px dashed #ffffff30;border-radius:var(--radius)}
.sk-App,.sk-Page{max-width:100%;margin:auto}.sk-Container,.sk-Card{padding:var(--space);border:1px solid #ffffff20;border-radius:var(--radius);background:#ffffff0b;box-shadow:var(--shadow)}.sk-Row{display:flex;flex-wrap:wrap;gap:.75rem;align-items:center}.sk-Column{display:flex;flex-direction:column;gap:.75rem}.sk-Grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:.75rem}
h1,h2,h3,h4,h5,h6,p{margin:.35rem 0;overflow-wrap:anywhere}button,input,textarea,select{font:inherit}button{border:0;border-radius:.7rem;padding:.7rem 1rem;background:linear-gradient(90deg,var(--c-primary),var(--c-primary2));color:white;font-weight:700;cursor:pointer}button.secondary{background:#334155}button.danger{background:var(--c-danger)}button:disabled,input:disabled{opacity:.5;cursor:not-allowed}
input,textarea,select{width:100%;border:1px solid #ffffff2f;border-radius:.65rem;padding:.7rem;background:#02061780;color:white;outline:none}input:focus,textarea:focus,select:focus{border-color:#8b5cf6;box-shadow:0 0 0 3px #8b5cf630}.field{display:flex;gap:.5rem;align-items:center}.field input[type=checkbox],.field input[type=radio]{width:auto}.sk-Alert{padding:.75rem;border-radius:.65rem;background:#0ea5e922;border:1px solid #38bdf855}.sk-Badge{display:inline-flex;width:max-content;padding:.25rem .6rem;border-radius:999px;background:#7c3aed;color:white}.sk-Image{max-width:100%;border-radius:.65rem}.sk-Progress{width:100%;accent-color:var(--c-primary)}canvas{max-width:100%;height:auto;background:#fff;border-radius:.65rem}.sk-Modal{position:fixed;inset:10%;z-index:10;overflow:auto;padding:1rem;border-radius:var(--radius);background:#172033;border:1px solid #ffffff30;box-shadow:0 30px 80px #000b}.sk-Accordion details,.sk-Tabs section{padding:.6rem;border:1px solid #ffffff20;border-radius:.6rem}.sk-Table{width:100%;border-collapse:collapse}.sk-Table td,.sk-Table th{padding:.5rem;border:1px solid #ffffff20}.timer{font-variant-numeric:tabular-nums;font-size:1.5rem;font-weight:800}.error{color:#fecaca;background:#7f1d1d66;padding:1rem;border-radius:.7rem}
</style>
</head>
<body><main id="root" class="root"><div class="empty">اضغط «تشغيل» لعرض التطبيق</div></main>
<script>
"use strict";
var currentUi=null,timers=[];
function send(id,event,value){parent.postMessage({source:"skui-preview",type:"event",id:id,event:event,value:value},"*")}
...
function bindCommon(el,node){
 ...
 ["focus","blur"].forEach(function(ev){el.addEventListener(ev,function(){send(id,"on_"+ev,p.value)})});
 el.addEventListener("keydown",function(e){if(e.key==="Enter")send(id,"on_key_press",e.key)});
 safeStyle(el,p);if(p.disabled)el.disabled=true;
}
function canvasNode(node){
 var p=node.props||{},cv=document.createElement("canvas");cv.className="sk-Canvas";cv.width=Math.min(1600,Number(p.width)||480);cv.height=Math.min(1000,Number(p.height)||280);
...
function make(node){
 var p=node.props||{},el;
 if(node.type==="Heading"){var level=Math.max(1,Math.min(6,Number(p.level)||1));el=document.createElement("h"+level);el.textContent=text(p.text)}
 ...
 else if(node.type==="Input"||node.type==="TextArea"){el=document.createElement(node.type==="TextArea"?"textarea":"input");el.placeholder=text(p.placeholder);el.value=text(p.value);if(node.type==="TextArea")el.rows=Math.max(1,Math.min(20,Number(p.rows)||4));el.addEventListener("input",function(){send(node.id,"on_input",el.value)});el.addEventListener("change",function(){send(node.id,"on_change",el.value)})}
 else if(node.type==="Checkbox"||node.type==="Radio"){el=document.createElement("label");el.className="field";var i=document.createElement("input");i.type=node.type.toLowerCase();i.checked=Boolean(p.checked||p.value===true);if(node.type==="Radio")i.name=text(p.group);i.addEventListener("change",function(){send(node.id,"on_change",node.type==="Checkbox"?i.checked:p.value)});var s=document.createElement("span");s.textContent=text(p.text);el.append(i,s)}
 else if(node.type==="Select"){el=document.createElement("select");(p.options||[]).forEach(function(item){var o=document.createElement("option");if(item&&typeof item==="object"){o.value=text(item.value);o.textContent=text(item.label||item.value)}else{o.value=text(item);o.textContent=text(item)}if(o.value===text(p.value))o.selected=true;el.appendChild(o)});el.addEventListener("change",function(){send(node.id,"on_select",el.value);send(node.id,"on_change",el.value)})}
 else if(node.type==="Slider"){el=document.createElement("input");el.type="range";el.min=Number(p.min)||0;el.max=Number(p.max)||100;el.step=Number(p.step)||1;el.value=Number(p.value)||0;el.addEventListener("input",function(){send(node.id,"on_input",Number(el.value))});el.addEventListener("change",function(){send(node.id,"on_change",Number(el.value))})}
 ...
 el.classList.add("sk-"+node.type);bindCommon(el,node);return el;
}
  ...
  useEffect(() => {
    const handler = (event) => {
      ...
      if (message.source === "skui-preview" && message.type === "event") {
        onEvent?.(message.id, message.event, message.value);
      }
    };
