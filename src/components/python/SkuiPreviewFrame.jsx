...
function make(node){
 ...
 el.classList.add("sk-"+node.type);if(/^(secondary|danger|operator|success|ghost)$/.test(p.variant||""))el.classList.add("variant-"+p.variant);if(/^(sm|md|lg)$/.test(p.size||""))el.classList.add("size-"+p.size);if(p.direction==="ltr"||p.direction==="rtl")el.dir=p.direction;bindCommon(el,node);return el;
}
