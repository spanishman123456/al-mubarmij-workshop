...
import { KOCH_CHALLENGES, checkFractalLabAnswer } from "../../lib/algorithms/fractals.js";
...
export function KochSnowflakeLab({ lessonId, userId }) {
  const { progress, restored, persist, markComplete } = useLessonProgress({
    studentId: userId,
    lessonId,
    sectionId: "koch-snowflake-lab",
  });
  ...
  const challenge = useMemo(
    () => KOCH_CHALLENGES[challengeIndex] || KOCH_CHALLENGES[0],
    [challengeIndex],
  );
  ...
  function checkAnswer() {
    ...
    if (result.ok) {
      ...
      setFeedback("غير صحيح — كل قطعة خط تُقسّم إلى 4 قطع في كل تكرار.");
      save({ answer });
    }
  }
  ...
  function revealHint() {
    ...
    setHints(next);
    if (next === 1) {
      setFeedback("تلميح 1: في كل عمق، عدد القطع يتضاعف بمعامل 4 لكل قطعة موجودة.");
    } else {
  ...
  return (
    <div
      ...
      dir="rtl"
      data-testid="koch-snowflake-lab"
    >
      <p className="font-bold text-violet-900">مختبر ندفة Koch</p>
      <p className="mt-1 text-sm text-slate-700">
        احسب عدد قطع الخط بعد تكرارات منحنى Koch على مثلث بثلاثة أضلاع.
      </p>

      <div className="mt-3 flex flex-wrap gap-2">
        {KOCH_CHALLENGES.map((c, i) => (
          <button
            key={c.id}
            type="button"
