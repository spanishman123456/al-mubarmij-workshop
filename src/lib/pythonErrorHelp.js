...
function classifyAndHint(text, opts = {}) {
  ...
  if (/expected an indented block/i.test(t)) {
    return {
      ...
      hintAr:
        "بعد if/for/while/def يجب أن يأتي سطر جديد بمسافة إضافية (4 مسافات). استخدم زر «إصلاح المسافات تلقائيًا» ثم راجع المنطق.",
    };
  }
  ...
  if (/indentationerror|unexpected indent|unindent/i.test(t)) {
    return {
      ...
      hintAr:
        "في بايثون يجب أن تكون المسافات متسقة (4 مسافات لكل مستوى). استخدم Tab للإزاحة، أو زر «إصلاح المسافات تلقائيًا» لتصحيح التنسيق.",
    };
  }
