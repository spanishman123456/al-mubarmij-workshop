/**
 * حلول skui الكاملة — للمعلمين فقط عبر API محمي.
 * لا تُستورد من حزمة العميل (Vite).
 */

export const SKUI_TEACHER_SOLUTIONS = {
...
guide = ui.Guide(
    title="مرحبًا بك في الآلة الحاسبة",
    message="اختر الأرقام ثم العملية. استخدم C للمسح و⌫ للحذف. يمكنك الكتابة من لوحة المفاتيح.",
    character="assistant",
    position="top",
)
...
def push_history(line):
    ...
    history.set_text(" | ".join(history_list[:4]) if history_list else "—")
...
def calculate():
    ...
    if symbol == "+":
        ...
        return
    line = str(a) + " " + symbol + " " + str(b) + " = " + str(answer)
    push_history(line)
    set_display(str(answer))
    status.set_text("تم الحساب")
    left[0] = answer
    op[0] = ""
    expr[0] = ""
...
display = ui.Input(placeholder="0", value="", size="lg", direction="ltr", on_key_press=on_key)
